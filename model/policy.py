...
class DefaultPolicy(nn.Module):
    def __init__(self, hidden_size_pol, hidden_size, persona_size, senti_size):
    ...
    def forward(self, encodings, persona_tensor, senti_tensor, act_tensor=None):
        ...
        # Network based
        output = self.W_u(hidden[0]) + self.W_persona(persona_tensor) + self.W_senti(senti_tensor)
