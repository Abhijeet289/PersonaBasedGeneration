...
class Model(nn.Module):
    ...
    def build_model(self):
        ...
        self.policy = policy.DefaultPolicy(self.hid_size_pol, self.hid_size_enc, self.db_size, self.bs_size, self.persona_size).to(self.device)
    ...
    def train(self, input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor, persona_tensor, dial_name=None):
        proba, _, decoded_sent = self.forward(input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor, persona_tensor)
    ...
    def forward(self, input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor, persona_tensor):
        ...
        # POLICY
        decoder_hidden = self.policy(encoder_hidden, db_tensor, bs_tensor, persona_tensor)
    ...
    def predict(self, input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor, persona_tensor):
        with torch.no_grad():
            # ENCODER
            encoder_outputs, encoder_hidden = self.encoder(input_tensor, input_lengths)
