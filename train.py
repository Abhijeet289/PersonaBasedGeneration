...
def train(print_loss_total,print_act_total, print_grad_total, input_tensor, target_tensor, bs_tensor, name=None):
    ...
    loss, loss_acts, grad = model.train(input_tensor, input_lengths, target_tensor, target_lengths,
                             bs_tensor, name)
...
def trainIters(model, n_epochs=10, args=args):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        input_tensor = [];target_tensor = [];bs_tensor = [];
        for name in dials:
            val_file = train_dials[name]
            model.optimizer.zero_grad()
            model.optimizer_policy.zero_grad()

            input_tensor, target_tensor, bs_tensor = util.loadDialogue(model, val_file, input_tensor, target_tensor, bs_tensor)

            print_loss_total, print_act_total, print_grad_total = train(print_loss_total, print_act_total, print_grad_total, input_tensor, target_tensor, bs_tensor)
            input_tensor = [];target_tensor = [];bs_tensor = [];
        ...
        for name, val_file in val_dials.items():
            input_tensor = []; target_tensor = []; bs_tensor = [];
            input_tensor, target_tensor, bs_tensor = util.loadDialogue(model, val_file, input_tensor,
            ...
            proba, _, _ = model.forward(input_tensor, input_lengths, target_tensor, target_lengths, bs_tensor)
