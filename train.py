...
def train(print_loss_total,print_act_total, print_grad_total, input_tensor, target_tensor, bs_tensor, db_tensor, persona_tensor, name=None):
...
def trainIters(model, n_epochs=10, args=args):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        input_tensor = [];target_tensor = [];bs_tensor = [];db_tensor = [];persona_tensor = [];
        for name in dials:
            val_file = train_dials[name]
            model.optimizer.zero_grad()
            model.optimizer_policy.zero_grad()

            input_tensor, target_tensor, bs_tensor, db_tensor, persona_tensor = util.loadDialogue(model, val_file, input_tensor, target_tensor, bs_tensor, db_tensor, persona_tensor)

            print_loss_total, print_act_total, print_grad_total = train(print_loss_total, print_act_total, print_grad_total, input_tensor, target_tensor, bs_tensor, db_tensor, persona_tensor)
            input_tensor = [];target_tensor = [];bs_tensor = [];db_tensor = [];persona_tensor = [];
        ...
        for name, val_file in val_dials.items():
            input_tensor = []; target_tensor = []; bs_tensor = [];db_tensor = [];persona_tensor = [];
            input_tensor, target_tensor, bs_tensor, db_tensor, persona_tensor = util.loadDialogue(model, val_file, input_tensor,
                                                                                         target_tensor, bs_tensor,
                                                                                         db_tensor, persona_tensor)
            # create an empty matrix with padding tokens
            input_tensor, input_lengths = util.padSequence(input_tensor)
            target_tensor, target_lengths = util.padSequence(target_tensor)
            bs_tensor = torch.tensor(bs_tensor, dtype=torch.float, device=device)
            ...
            proba, _, _ = model.forward(input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor, persona_tensor)
